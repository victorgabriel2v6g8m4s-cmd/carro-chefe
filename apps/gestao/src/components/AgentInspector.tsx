...
export function AgentInspector({ agentId, onClose }: { agentId: string | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    api<AgentStats>(`/api/v1/agents/${agentId}/stats`).then(setStats).catch((cause) => setError(cause instanceof Error ? cause.message : String(cause)));
  }, [agentId]);
  ...
  return <div className="inspector-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <aside className="agent-inspector" role="dialog" aria-modal="true" aria-labelledby="agent-inspector-title">
      ...
      {!stats ? <p className="loading">{error || "Calculando desempenho…"}</p> : <>
        ...
        <div className="agent-settings"><div><small>Modelo</small><strong>{stats.agent.model || "Padrão do Codex"}</strong></div><div><small>Raciocínio</small><strong>{stats.agent.reasoningEffort}</strong></div><div><small>Navegador</small><strong>{stats.agent.browserEnabled ? "Habilitado" : "Desabilitado"}</strong></div><div><small>Workspace</small><strong>{stats.agent.workspaceMode === "project" ? "Projeto completo" : stats.agent.workspaceMode === "artifacts" ? "Artefatos isolados" : "Somente leitura"}</strong></div></div>
        ...
        <h3>Execuções recentes</h3><div className="mini-run-list">{stats.recentRuns.length ? stats.recentRuns.map((run) => <Link key={run.id} to={`/gestao/agentes/execucoes/${run.id}`} onClick={onClose}><span>{run.title}</span><StatusBadge status={run.outcome || run.status} /></Link>) : <p>Nenhuma execução registrada.</p>}</div>
      </>}
    </aside>
  </div>;
}
