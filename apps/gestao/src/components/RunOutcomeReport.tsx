...
function ResultList({ title, items, tone }: { title: string; items: string[]; tone: string }) {
  return <section className={`outcome-column outcome-column--${tone}`}><div><i /> <strong>{title}</strong><span>{items.length}</span></div>{items.length ? <ul>{items.map((item, index) => <li key={`${tone}-${index}`}><RichReferences text={item} /></li>)}</ul> : <p>Nenhum item registrado.</p>}</section>;
}
...
export function RunOutcomeReport({ report }: { report: any }) {
  ...
  return <section className={`panel outcome-report outcome-report--${report.outcome}`}>
    ...
    {report.evidence?.length > 0 && <details className="report-evidence"><summary>Evidências usadas no diagnóstico</summary><ul>{report.evidence.map((item: string, index: number) => <li key={index}><RichReferences text={item} /></li>)}</ul></details>}
