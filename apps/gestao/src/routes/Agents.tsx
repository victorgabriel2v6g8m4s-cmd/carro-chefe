...
export function Agents() {
  ...
  useEffect(() => {
    ...
    if (!task) return;
    setAgentId(task.ownerAgentId);
    setTitle(`Executar ${task.id} — ${task.title}`);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const run = await api<any>("/api/v1/agent-runs", json("POST", { taskId, agentId, title, objective, provider, requestedBy: "proprietario" }));
      setFeedback(`Execução ${run.id} criada e adicionada à fila.`);
      await refresh();
