...
export function IntegratedBrowser() {
  ...
  const isInternalViewer = current.startsWith("/gestao/visualizador");
  const frameUrl = useMemo(() => isInternalViewer ? current : current, [current, isInternalViewer]);
  ...
  return <div className="page-stack browser-page"><section className="intro"><span className="eyebrow">Navegação assistida</span><h2>Navegador integrado dos agentes</h2><p>O agente pode conduzir esta área até a página ou arquivo exato da execução. Ações em sessões pessoais continuam sob seu controle.</p></section>
    ...
    <section className="browser-workspace"><div className="browser-frame panel"><div className="browser-chrome"><i /><i /><i /><span>{current}</span><a href={current} target="_blank" rel="noreferrer">Abrir fora ↗</a></div><iframe title="Navegador integrado" src={frameUrl} sandbox="allow-forms allow-modals allow-popups allow-same-origin allow-scripts" /></div>
