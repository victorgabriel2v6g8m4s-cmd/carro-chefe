...
export function Tasks() {
  ...
  return <div className="page-stack">
    <TaskCreatePanel />
    <section className="intro"><span className="eyebrow">Plano vivo</span><h2>Tarefas e responsabilidades</h2><p>Os filtros fazem parte do endereço da página: você pode recarregar ou compartilhar sem perder o contexto.</p></section>
    <section className="panel filters">
      <label><span>Buscar</span><input value={params.get("q") ?? ""} onChange={(event) => update("q", event.target.value)} placeholder="Título ou código" /></label>
      <label><span>Status</span><select value={status} onChange={(event) => update("status", event.target.value)}><option value="">Todos</option><option value="ready">Prontas</option><option value="in_progress">Em andamento</option><option value="blocked">Bloqueadas</option><option value="review">Em revisão</option><option value="done">Concluídas</option></select></label>
      <label><span>Responsável</span><select value={owner} onChange={(event) => update("responsavel", event.target.value)}><option value="">Todos</option>{data.project.agents.map((agent: any) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label>
    </section>
