...
export function CommandDetail() {
  ...
  return <div className="page-stack">
    ...
    {intent.uploads?.length > 0 && <section className="run-attachments"><span>Anexos do comando e artefatos produzidos</span>{intent.uploads.map((upload: any) => <Link key={upload.id} to={`/gestao/visualizador?uploadId=${encodeURIComponent(upload.id)}`}><strong>{upload.originalName}</strong><small>{Math.ceil(upload.sizeBytes / 1024)} KB · {upload.runId ? "resultado" : "entrada"}</small></Link>)}</section>}
