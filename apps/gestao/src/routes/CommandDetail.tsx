...
export function CommandDetail() {
  ...
  useEffect(() => {
    ...
    ["intent.started", "intent.completed", "intent.failed", "agent.run.updated", "agent.step.updated", "agent.message.created", "agent.question.asked", "agent.log.created"].forEach((name) => stream.addEventListener(name, update));
    return () => stream.close();
  }, [intentId]);
