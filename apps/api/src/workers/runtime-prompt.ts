...
export function buildRunPrompt(run: any, handoffs: string[], latestAnswer: any, ownerAnswers: Array<{ question: string; answer: string }> = [], workspace: RuntimeWorkspace, knowledge: any[] = []) {
  ...
  const chatHistory = run.purpose === "management_chat" && !run.externalThreadId
    ...
    : "";
  if (run.purpose === "management_chat") return `Você é ${run.agent.name} (${run.agent.id}), responsável pela coordenação central. Responda ao proprietário sem criar uma tarefa.\nMensagem: ${run.objective}${chatHistory ? `\nContexto recente da conversa:\n${chatHistory}` : ""}\nVocê pode consultar especialistas com o comando send. Marque --required apenas quando depender da resposta; as demais consultas serão acumuladas e despachadas em lote para manter o contexto e economizar tokens.${decisions}${memory}${workspaceRule}${contract}`;
