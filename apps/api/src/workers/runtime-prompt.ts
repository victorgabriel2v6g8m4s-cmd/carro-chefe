...
export function buildRunPrompt(run: any, handoffs: string[], latestAnswer: any, ownerAnswers: Array<{ question: string; answer: string }> = [], artifactAuthoring = false) {
  ...
  const artifactRule = artifactAuthoring ? "\nEsta execução está autorizada a criar o artefato solicitado. Agentes não técnicos podem escrever somente em output/ e tmp/; não edite código, configuração ou documentação do projeto. Registre o arquivo final no endpoint /artifacts." : "";
  if (latestAnswer) return `Resposta do proprietário para “${latestAnswer.question}”: ${latestAnswer.answer}. Continue do ponto salvo e registre somente novidades.${decisions}${artifactRule}${contract}`;
  return `Agente: ${run.agent.name} (${run.agent.id}). Missão exclusiva: ${run.agent.mission}\nObjetivo: ${run.objective}\nTarefa ${run.task.id}: ${run.task.title}\nStatus: ${run.task.status}${run.task.statusJustification ? ` — ${run.task.statusJustification}` : ""}\nAceite: ${run.task.acceptance}${handoffs.length ? `\nHandoffs relevantes:\n${handoffs.join("\n")}` : ""}${decisions}\nPerfil: complexidade ${run.complexity}; ${run.routingReason || "política adaptativa"}.\nNão trabalhe fora da sua especialidade. Agentes não técnicos não editam código; programação pertence ao AG-DEV. AG-GESTAO monitora, prioriza e decide, sem repetir especialistas. Não compre, publique, faça deploy ou altere serviços externos sem autorização. Use o mínimo de leituras e chamadas.${artifactRule}${contract}`;
}
