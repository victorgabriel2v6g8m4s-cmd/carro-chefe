...
import { formatPolicyPrompt, type AgentPolicyContext } from "./policy-context";
...
export function buildRunPrompt(run: any, handoffs: string[], latestAnswer: any, ownerAnswers: Array<{ question: string; answer: string }> = [], workspace: RuntimeWorkspace, knowledge: any[] = [], policy: AgentPolicyContext) {
  const commands = workspace.mode === "artifacts"
    ? { send: ".\\send", question: ".\\ask", artifact: ".\\artifact", remember: ".\\remember", recall: ".\\recall" }
    : { send: `node tools/agent-runtime.mjs send ${run.id}`, question: `node tools/agent-runtime.mjs question ${run.id}`, artifact: `node tools/agent-runtime.mjs artifact ${run.id}`,
      remember: `node tools/agent-runtime.mjs remember ${run.id}`, recall: `node tools/agent-runtime.mjs recall ${run.id}` };
  ...
  const memory = knowledge.length ? `\nMemória operacional relevante (selecionada por caminho):\n${knowledge.map((item) => `- ${item.path}: ${(item.value ?? item.name).slice(0, 320)}`).join("\n")}` : "";
  const policyRule = formatPolicyPrompt(policy);
  ...
  if (latestAnswer) {
    ...
    return `Resposta do proprietário para “${latestAnswer.question}”: ${answerContext}. Continue do ponto salvo e registre somente novidades.${decisions}${memory}${dispatchResults.length ? `\nPareceres recebidos:\n${dispatchResults.join("\n")}` : ""}${policyRule}${workspaceRule}${contract}`;
  }
  ...
  const chatHistory = run.purpose === "management_chat" && !run.externalThreadId
    ...
    : "";
  if (run.purpose === "management_chat") return `Você é ${run.agent.name} (${run.agent.id}), responsável pela coordenação central. Responda ao proprietário sem criar uma tarefa.\nMensagem: ${run.objective}${chatHistory ? `\nContexto recente da conversa:\n${chatHistory}` : ""}\nVocê pode consultar especialistas com o comando send. Marque --required apenas quando depender da resposta; as demais consultas serão acumuladas e despachadas em lote para manter o contexto e economizar tokens.${decisions}${memory}${policyRule}${workspaceRule}${contract}`;
