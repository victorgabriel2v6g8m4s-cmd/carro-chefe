...
export async function runtimeApi<T>(route: string, method = "GET", body?: unknown): Promise<T> {
  const response = await fetch(`${runtimeApiBase}${route}`, { method, headers: { "Content-Type": "application/json; charset=utf-8", ...(config.agentApiKey ? { "X-Agent-Key": config.agentApiKey } : {}) }, body: body === undefined ? undefined : JSON.stringify(body) });
