...
| `excel-recipe` | `tools/excel_recipe` | motor reutilizável de receitas JSON transacionais para `.xlsm` compatíveis; CRUD V1, mapa/rename V2, V3A estrutural, SHA/firewall/rollback, receipts por workbook, snapshot opt-in genérico e integração legada do Carro Chefe | completa | médio/alto em refactors estruturais | nenhum direto/baixo | G1 ainda restringe o workspace ao repositório e aceita apenas `.xlsm`; fail-closed; não executa VBA nem recalcula fórmulas; Drawing/VML/ActiveX, gráficos, pivôs, Power Query/conexões e sintaxes não regraváveis permanecem blockers | 97 |
