...
| `excel-recipe` | `tools/excel_recipe` | receitas JSON transacionais no XLSM; CRUD V1, mapa/rename V2 e V3A com insert/delete físico de linhas/colunas, `range.move`, inserção/exclusão de coluna no meio de Table, compactação física, plano determinístico, rollback, snapshot e receipts | completa | médio/alto em refactors estruturais | nenhum direto/baixo | fail-closed; não executa VBA nem recalcula fórmulas; Drawing/VML/ActiveX, gráficos, pivôs, Power Query/conexões e sintaxes não regraváveis permanecem blockers | 97 |
