...
| `excel-recipe` | `tools/excel_recipe` | validar, planejar e aplicar receitas JSON transacionais no XLSM; scanner de dependências, refactors conservadores, rollback, snapshot e receipts auditáveis | completa | médio | nenhum direto/baixo | não executa VBA nem recalcula fórmulas; não edita VBA/ActiveX, gráficos, pivôs, Power Query/conexões; bloqueia dependências ambíguas | 96 |
