...
| `excel-recipe` | `tools/excel_recipe` | receitas JSON transacionais no XLSM; CRUD V1, mapa/rename V2, estrutura física V3A e V3B com reescrita controlada de `oneCellAnchor`/`twoCellAnchor` e referências A1 de gráficos clássicos; plano determinístico, rollback, snapshot e receipts | completa | médio/alto em refactors estruturais | nenhum direto/baixo | fail-closed; não executa VBA nem recalcula fórmulas; VML/ActiveX/OLE, PivotChart/pivôs, charts não suportados, mudanças de cardinalidade de cache, Power Query/conexões e sintaxes não regraváveis permanecem blockers | 98 |
