...
```mermaid
flowchart LR
    Alcance["Alcance local<br/>rua + Instagram + anúncios"] --> Interesse["Interesse<br/>vídeo do preparo + produto"]
    Interesse --> Intencao["Intenção<br/>/welcome + cardápio + WhatsApp"]
    Intencao --> Pedido["Pedido pago<br/>ERP"]
    Pedido --> Experiencia["Produto + atendimento + tempo"]
    Experiencia --> Retencao["Recompra + avaliação + indicação"]
```
